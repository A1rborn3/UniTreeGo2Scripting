...
class Go2KeyboardController(SportClient, ObstaclesAvoidClient):
    def __init__(self, network_interface: str = "lo"):#change to 'lo'/'eth0' for local or connected
        SportClient.__init__(self)
        ObstaclesAvoidClient.__init__(self) #setup sport client and obi avoid

        self.SetTimeout(10.0)
        SportClient.Init(self)  # Initialize sport client
        ObstaclesAvoidClient.Init(self)# Initialize obstacle avoidance client
    ...
    def enable_obstacle_avoidance(self):
        self.UseRemoteCommandFromApi(True)  # Tell system to accept API commands
        self.SwitchSet(True)  # Enable obstacle avoidance
        self.obstacle_avoidance_enabled = True
        time.sleep(0.5)

    def disable_obstacle_avoidance(self):
        self.SwitchSet(False)
        self.UseRemoteCommandFromApi(False)
        self.obstacle_avoidance_enabled = False
    ...
    def run(self, stdscr):
        ...
        try:
            while True:
                ...
                if ord('w') in self.pressed_keys: vx += 0.5
                if ord('s') in self.pressed_keys: vx -= 0.5
                if ord('q') in self.pressed_keys: vy += 0.3
                if ord('e') in self.pressed_keys: vy -= 0.3
                if ord('a') in self.pressed_keys: vyaw += 0.5
                if ord('d') in self.pressed_keys: vyaw -= 0.5
                
                if curses.KEY_UP in self.pressed_keys: pitch += 0.2
                if curses.KEY_DOWN in self.pressed_keys: pitch -= 0.2
                if curses.KEY_LEFT in self.pressed_keys: yaw += 0.3
                if curses.KEY_RIGHT in self.pressed_keys: yaw -= 0.3
                
                #Send to Robot
                if self.is_standing:
                    # Use ObstaclesAvoidClient.Move() when obstacle avoidance is enabled
                    if self.obstacle_avoidance_enabled:
                        ObstaclesAvoidClient.Move(self, vx, vy, vyaw)  #ObstaclesAvoidClient.Move()
                    else:
                        SportClient.Move(self, vx, vy, vyaw)  #Explicitly call SportClient.Move()
...
#run command |python3 go2_keyboard_controller.py eth0